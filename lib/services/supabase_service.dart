...
class SupabaseService {
  ...
  // Leer lista completa de películas
  Future<List<Map<String, dynamic>>> getWatchlist() async {
    final response = await client.from('watchlist').select('id, title');
    return (response as List).cast<Map<String, dynamic>>();
  }

  // Agregar una película
  Future<void> addToWatchlist(String title) async {
    await client.from('watchlist').insert({'title': title});
  }
...
Future<List<Map<String, dynamic>>> getFavorites() async {
  final response = await client.from('favorites').select('id, title');
  return (response as List).cast<Map<String, dynamic>>();
}

Future<void> addFavorite(String title) async {
  await client.from('favorites').insert({'title': title});
}
